...
def visualize_mesh(mesh: trimesh.Trimesh, front_points, front_normals=None, back_points=None, back_normals=None):
    # Create a scene to display
    scene = trimesh.Scene()

    # Add the faucet target link mesh to the scene
    scene.add_geometry(mesh)

    # display centroid in blue
    centroid = mesh.centroid
    centroid = np.array(centroid)
    centroids = np.tile(centroid, (10,1))
    contact_points_visual = trimesh.points.PointCloud(centroids, colors=[0, 0, 255])
    scene.add_geometry(contact_points_visual)

    # display front points in red
    contact_points_visual = trimesh.points.PointCloud(front_points, colors=[255, 0, 0])
    scene.add_geometry(contact_points_visual)

    # display back points in green
    if back_points is not None:
        contact_points_visual = trimesh.points.PointCloud(back_points, colors=[0, 255, 0])
        scene.add_geometry(contact_points_visual)

    # For each contact point, create a line representing the normal
    if front_normals is not None:
        for center, normal in zip(front_points, front_normals):
            # Scale and create line segments for normals
            vec = np.array([center, center + (normal * 0.01)])
            path = trimesh.load_path(vec.reshape((-1, 2, 3)))
            scene.add_geometry(path)

    if back_normals is not None:
        for center, normal in zip(back_points, back_normals):
            # Scale and create line segments for normals
            vec = np.array([center, center + (normal * 0.01)])
            path = trimesh.load_path(vec.reshape((-1, 2, 3)))
            scene.add_geometry(path)
...
def visualize_facets(mesh: trimesh.Trimesh, contact_points, front_points, back_points):
    # Create a scene to display
    scene = trimesh.Scene()
    ...
    # Add contact points to the scene
    # You might want to adjust the color and point size
    contact_points_visual = trimesh.points.PointCloud(contact_points, colors=[255, 0, 0])
    scene.add_geometry(contact_points_visual)

    contact_points_visual = trimesh.points.PointCloud(front_points, colors=[0, 255, 0])
    scene.add_geometry(contact_points_visual)

    contact_points_visual = trimesh.points.PointCloud(back_points, colors=[0, 0, 255])
    scene.add_geometry(contact_points_visual)

    facets = mesh.facets  # facets are ids of the triangles that are coplanar ---> [[1,2,3,4],[5,15],[*,*,*],...]
    facets_normals = mesh.facets_normal  # normals of the facets (one normal per group)
    scale_factor = 0.05
    facet_centers = np.array([mesh.triangles_center[facet].mean(axis=0) for facet in facets])
    for center, normal in zip(facet_centers, facets_normals):
        # Scale and create line segments for facet normals
        vec = np.array([center, center + (normal * scale_factor)])
        path = trimesh.load_path(vec.reshape((-1, 2, 3)))
        scene.add_geometry(path)
