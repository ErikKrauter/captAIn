...
from mani_skill2.utils.trimesh_utils import get_actor_mesh, visualize_mesh, visualize_facets, get_actor_visual_mesh
...
class TurnFaucetBaseEnv(BaseEnv):
    ...
    def __init__(
        ...
        *args,
        ...
        front_cam=True,
        back_cam=False,
        offset_cameras=False,
        **kwargs,
    ):
        self.robot_uid = robot
        self.robot_init_qpos_noise = robot_init_qpos_noise
        self._use_front_cam = front_cam
        self._use_back_cam = back_cam
        self._offset_cameras = offset_cameras
    ...
    def _load_agent(self):
        ...
        self.agent = agent_cls(
            self._scene, self._control_freq, self._control_mode, config=self._agent_cfg, interpolate=self._interpolate,
        )
    ...
    def _initialize_agent(self):
        if self.robot_uid == "panda":
            ...
            #)
            # print("TurnFaucet setting q pose and root pose")
            self.agent.reset(qpos)  # this sets all joints
            self.agent.robot.set_pose(Pose([-0.56, 0, 0]))  # this sets the root Pose
    ...
    def _register_cameras(self):
        if not self._offset_cameras:
            pose = look_at([-0.4, 0, 0.3], [0, 0, 0.1])
            pose_behind = look_at([0.4, 0, 0.5], [0, 0, 0.1])
        else:
            pose = look_at([-0.4, 0, 0.3], [0, 0, 0.1])
            pose_behind = look_at([0.4, 0, 0.5], [0, 0, 0.1])
        cam = []
        if self._use_front_cam:
            cam1 = CameraConfig("base_camera", pose.p, pose.q, 128, 128, np.pi / 2, 0.01, 10)
            cam.append(cam1)
        if self._use_back_cam:
            cam2 = CameraConfig("base_camera_behind", pose_behind.p, pose_behind.q, 128, 128, np.pi / 2, 0.01, 10)
            cam.append(cam2)
        return cam
...
class TurnFaucetEnv(TurnFaucetBaseEnv):
    ...
    def __init__(
        ...
        friction: float = 0.1,
        damping: float = 2.0,
        randomize_physical_properties: bool = False,
        **kwargs,
    ):
        self.asset_root = Path(format_path(asset_root))  # data/partnet_mobility/dataset
        ...
        # json.dump(model_json, sys.stdout, indent=4)
        ...
        self.stiffness = 0.0
        self.randomize_physical_properties = randomize_physical_properties
    ...
    def reset(self, seed=None, options=None):
        ...
        # print(f"turn_faucet model_id: {self.model_id}")
    ...
    def _load_articulations(self):
        ...
        for joint in self.faucet.get_active_joints():
            joint.set_friction(self.friction)  # 1
            joint.set_drive_property(self.stiffness, self.damping)  # 0, 10
    ...
    def _set_switch_links(self):
        ...
        for semantic in self.model_info["semantics"]:
            ...
            elif semantic[1] == 'static':
                static_link_names.append(semantic[0])
        if len(switch_link_names) == 0 or len(static_link_names) == 0:
            raise RuntimeError(self.model_id)
        self.switch_link_names = switch_link_names
        self.static_link_names = static_link_names

        self.switch_links, self.static_links = [], []
        ...
        for name in self.switch_link_names:
            ...
            # cache mesh
            link_mesh = get_actor_visual_mesh(link, False)
            # link_mesh.show()
            self.switch_links_mesh.append(link_mesh)

            # hardcode
            joint = all_joints[link.get_index()]
            if self.randomize_physical_properties:
                self.friction = self._episode_rng.uniform(0.05, 8*0.1)
                self.damping = self._episode_rng.uniform(1.0, 8*2.0)
                # print(f'friction: \t {self.friction} \n damping: \t {self.damping}')

            joint.set_friction(self.friction)  # 0.1
            joint.set_drive_property(self.stiffness, self.damping)  # 0.0, 2.0
            self.switch_joints.append(joint)

        self.static_links_mesh: List[trimesh.Trimesh] = []
        for name in self.static_link_names:
            link = get_entity_by_name(all_links, name)
            self.static_links.append(link)
            link_mesh = get_actor_visual_mesh(link, False)
            # link_mesh.show()
            self.static_links_mesh.append(link_mesh)
    ...
    def _set_target_link(self):
        ...
        self.joint_origin = self.target_joint.get_global_pose().p

        self.target_link_mesh: trimesh.Trimesh = self.switch_links_mesh[idx]

        # NOTE(jigu): trimesh uses np.random to sample
        self.target_link_pcd = trimesh.sample.sample_surface(
            self.target_link_mesh, 1024, seed=self._episode_seed
        )[0]

        n_static_links = len(self.static_link_names)
        s_idx = random_choice(np.arange(n_static_links), self._episode_rng)
        self.static_link: sapien.Link = self.static_links[s_idx]
        self.static_link_mesh: trimesh.Trimesh = self.static_links_mesh[s_idx]

        self.static_link_pcd = trimesh.sample.sample_surface(
            self.static_link_mesh, 256, seed=self._episode_seed
        )[0]

        # visualize_mesh(self.static_link_mesh, self.static_link_pcd)
