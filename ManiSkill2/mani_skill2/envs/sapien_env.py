...
class BaseEnv(gym.Env):
    ...
    def __init__(
        ...
        hand_held_cam: bool = True,
        bg_name: str = None,
    ):

        self._interpolate = interpolate
        self._render_robot = render_robot
        self._hand_held_cam = hand_held_cam
        ...
        obs, _ = self.reset(seed=self._main_seed, options=dict(reconfigure=True))
    ...
    # I need this to have direct access to the rng through the vec env class. Else I dont know how to guarantee
    # reproducibility
    def seed(self, seed):
    ...
    def _configure_cameras(self):
        ...
        self._agent_camera_cfgs = OrderedDict()  # this dict is necessary else maniskill complains
        if self._hand_held_cam:
            if self._agent_cfg is not None:
                if self._agent_cfg.cameras is not None:
    ...
    def reset(self, seed=None, options=None):
        # self.custom_print("sapien env resetting")
        if options is None:
            options = dict()
    ...
    def set_main_rng(self, seed):
        ...
        # print(f"self._main_seed: {self._main_seed}")
    ...
    def set_episode_rng(self, seed):
        ...
        # print(f"self._episode_seed: {self._episode_seed}")
    ...
    def step(self, action: Union[None, np.ndarray, Dict]):
        ...
        obs = self.get_obs()
        info = self.get_info(obs=obs, action=action)
