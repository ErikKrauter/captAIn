...
class PDEEPosController(PDJointPosController):
    ...
    def ee_pose_at_base(self):
        to_base = self.articulation.pose.inv()  # self.articulation is panda_v2
        return to_base.transform(self.ee_pose)

    def reset(self):
        super().reset()
        self._target_pose = self.ee_pose_at_base
        self._IK_error = False

    def compute_ik(self, target_pose, max_iterations=1000):
        # Assume the target pose is defined in the base frame
        # print(f"initial_qpos: {self.articulation.get_qpos()}")
        rotation_axis = Rotation.from_quat(target_pose.q[[1, 2, 3, 0]]).as_rotvec()
        angle = np.linalg.norm(rotation_axis)
        rotation_axis /= angle
        angle = np.rad2deg(angle)
        #print(f'BASE: target position base {target_pose.p}, target rotation axis base {rotation_axis}, target angle in deg: {angle}')
        result, success, error = self.pmodel.compute_inverse_kinematics(
            self.ee_link_idx,
            target_pose,
            initial_qpos=self.articulation.get_qpos(),  # that the robots joint angles
    ...
    def get_IK_error(self):
        return self._IK_error

    def set_action(self, action: np.ndarray):
        action = self._preprocess_action(action)  # this is the issue...
        ...
        if self._target_qpos is None:
            # print('INVERSE KINEMATICS RESULTS IN NONE!!')
            self._IK_error = True
