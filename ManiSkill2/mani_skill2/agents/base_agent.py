...
class BaseAgent:
    ...
    def __init__(
        ...
        # Controller
        self.controller_configs = self.config.controllers  # This is a reference to the def controllers function
        if interpolate and control_mode is not None:
            # Modify the 'arm_pd_ee_delta_pose' config
            arm_pd_ee_delta_pose_config = self.controller_configs[control_mode]['arm']
            arm_pd_ee_delta_pose_config.interpolate = True
            gripper_pd_ee_delta_pose_config = self.controller_configs[control_mode]['gripper']
            gripper_pd_ee_delta_pose_config.interpolate = True
            print("HAVE SET INTERPOLATION TO TRUE")

        self.supported_control_modes = list(self.controller_configs.keys())
        if control_mode is None:
            raise Exception('No control mode specified!')
