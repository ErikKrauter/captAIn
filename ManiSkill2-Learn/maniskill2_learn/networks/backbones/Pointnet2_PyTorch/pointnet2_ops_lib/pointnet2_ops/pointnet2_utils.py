...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        if torch.any(torch.isnan(idx)):
            get_logger().error(
                f"rank {get_world_rank()}: \t NANS in idx after ball_query in _PointnetSAModuleBase.QueryAndGroup")

        xyz_trans = xyz.transpose(1, 2).contiguous()
        grouped_xyz = grouping_operation(xyz_trans, idx)  # (B, 3, npoint, nsample)

        if torch.any(torch.isnan(grouped_xyz)):
            get_logger().error(
                f"rank {get_world_rank()}: \t NANS in grouped_xyz after grouping_operation in _PointnetSAModuleBase.QueryAndGroup")

        grouped_xyz -= new_xyz.transpose(1, 2).unsqueeze(-1)

        if features is not None:
            grouped_features = grouping_operation(features, idx)
            if torch.any(torch.isnan(grouped_features)):
                get_logger().error(
                    f"rank {get_world_rank()}: \t NANS in grouped_features after grouping_operation in _PointnetSAModuleBase.QueryAndGroup")
            if self.use_xyz:
