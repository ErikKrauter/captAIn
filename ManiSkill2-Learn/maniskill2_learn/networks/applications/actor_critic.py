...
class ActorCriticBase(ExtendedModule):
    ...
    def forward(self, obs, actions=None, **kwargs):
        ...
        if isinstance(obs, dict):
            if torch.any(torch.isnan(obs["xyz"])):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
                logger.warning(f"NANS in obs xyz of rank {rank}")
                logger.info(obs["xyz"])

            '''if torch.any(torch.isnan(obs["rgb"])):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
                logger.warning(f"NANS in obs rgb of rank {rank}")
                logger.info(obs["rgb"])'''
        elif isinstance(obs, torch.Tensor):
            if torch.any(torch.isnan(obs)):
                logger.warning(f"NANS in obs of rank {rank}")
                logger.info(obs)
        # actions are concatenated to "state" key of obs
        inputs = combine_obs_with_action(obs, actions)
        feature = self.backbone(inputs, **kwargs)  # NEED TO CHECK IF FEATURES ARE NAN

        if torch.any(torch.isnan(feature)):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
            logger.warning(f"NANS in feature output of backbone of rank {rank}")
            logger.info(feature)
        ...
        if self.head is not None:
            ...
            if isinstance(feature, (tuple, list)):
                for i in range(len(feature)):
                    if isinstance(feature[i], torch.Tensor):
                        if torch.any(torch.isnan(feature[i])):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
                            logger.warning(f"NANS in feature output of head of rank {rank} index {i}")
                            logger.info(feature)
            else:
                if isinstance(feature, torch.Tensor):
                    if torch.any(torch.isnan(feature)):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
                        logger.warning(f"NANS in feature output of head of rank {rank}")
                        logger.info(feature)
        ...
        if aux_loss is None or not kwargs.pop("require_aux_loss", False):
            ...
            return feature  # this is the action or the critic's state value
        else:
