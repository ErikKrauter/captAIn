...
class GaussianBaseHead(ContinuousBaseHead):
    ...
    def return_with_mean_std(self, mean, std, dist_builder, mode, logits=None):
        ...
        # import time
        # st = time.time()
        # self.logger.info(f"Rank {self.world_rank} building distribution on device {mean.device}")
        if torch.any(torch.isnan(mean)):
            self.logger.info("NANS IN MEAN")
        if torch.any(torch.isnan(std)):
            self.logger.info("NANS IN STD")
        dist = dist_builder(mean, std)  # CustomIndependent(ScaledTanhNormal(loc: torch.Size(B,7), scale: torch.Size(B,7)), 1)
        # print(1, time.time() - st)

        mean_ret = dist.mean  # Independent mean --> ScaledTanh mean --> ScaledTanh transform(Normal.meam)
        std_ret = dist.stddev  # CustomIndependent stddev --> Normal stddev
        if self.num_heads > 1:
...
class TanhGaussianHead(GaussianBaseHead):
    ...
    def forward(self, feature, num_samples=1, mode="explore", **kwargs):
        ...
        if torch.any(torch.isnan(feature)):  # NEED TO DO THE SAME CHECK FOR ALL OTHER KEYS IN OBS
            self.logger.warning(f"NANS in feature {self.world_rank}")
            self.logger.info(feature)
        feature = super(TanhGaussianHead, self).forward(feature)  # feature B, 14  # this doesnt do anything
        ...
        return self.return_with_mean_std(mean, std, dist_builder, mode, logits) # logits is None  # returns tensor B, 7 where 7 is action shape
