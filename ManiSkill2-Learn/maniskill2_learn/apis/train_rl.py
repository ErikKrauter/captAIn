...
import numpy as np
from maniskill2_learn.env import ReplayMemory, save_eval_statistics, DoubleEvaluation, SupervisedEvaluation
...
from maniskill2_learn.methods import VATPerception, PoseTrajectoryGenerator
import wandb
...
def train_rl(
    ...
    from maniskill2_learn.utils.torch import BaseAgent, BasePerception
    ...
    for iteration_id in itertools.count(1):
        # logger.error(f'rank {world_rank} in iteration {iteration_id}')
        ...
        if n_steps > 0:
            ...
            while num_episodes < print_steps and not (on_policy and num_episodes > 0):
                ...
                # logger.info(f"ENTER rollout")
                trajectories = rollout.forward_with_policy(agent, n_steps, on_policy, replay)  # dict 8: obs dict 5: xyz, rgb ..., next_obs, actions, rewards, dones, episode_dones, infos, worker_indices
                # logger.info(f"EXIT rollout")
                ...
                if trajectories is not None:
                    ...
                    episode_statistics.push(trajectories)
                    n_ep = np.sum(trajectories["episode_dones"].astype(np.int32)) # understand under what condition episode_dones are true
                    num_episodes += n_ep
                ...
                if hasattr(agent, 'update_perception'):
                    if agent.continuous_learning:
                        assert recent_traj_replay is not None
                        start_time = time.time()
                        update_applied, perception_training_info = agent.update_perception(recent_traj_replay, n_ep)
                        if update_applied:
                            recent_traj_replay.reset()
                            for key in perception_training_info:
                                tb_print[key].append(perception_training_info[key])
                        update_time += time.time() - start_time
            ...
            # For offline RL
            # logger.error(f'rank {world_rank}: starting for loop of offline training')
            for i in range(n_updates):
                total_updates += 1
                steps += 1
        ...
        if check_eval.check(steps) and is_not_null(evaluator):
            ...
            logger.error(f"Begin to evaluate at step: {steps}. " f"The evaluation info will be saved at eval_{standardized_eval_step}")
            eval_dir = work_dir  # osp.join(work_dir, f"eval_{standardized_eval_step}")

            agent.eval()  # For things like batch norm
            agent.set_mode(mode="test")  # For things like obs normalization

            if isinstance(evaluator, SupervisedEvaluation):

                eval_dict = evaluator.run(agent, **eval_cfg, work_dir=eval_dir, wandb_run=run)
                # in case of VAT we do multi-GPU evaluation, which means we need to do an allreduce of the results
                if agent.mode == 'affordancePredictor':
                    eval_dict = GDict(eval_dict).allreduce(wrapper=False)
            elif isinstance(evaluator, DoubleEvaluation):
                lens_id, rewards_id, finishes_id, lens_ood, rewards_ood, finishes_ood = evaluator.run(agent, **eval_cfg, work_dir=eval_dir, wandb_run=run)
                eval_dict = dict(
                                id_mean_length=np.mean(lens_id),
                                id_std_length=np.std(lens_id),
                                id_mean_reward=np.mean(rewards_id),
                                id_std_reward=np.std(rewards_id),
                                id_success_mean=np.mean(finishes_id),
                                ood_mean_length=np.mean(lens_ood),
                                ood_std_length=np.std(lens_ood),
                                ood_mean_reward=np.mean(rewards_ood),
                                ood_std_reward=np.std(rewards_ood),
                                ood_success_mean=np.mean(finishes_ood)
                                 )
            else:
                # in all other cases the evaluation requires environment interactions.
                # we will only do that on GPU 0
                lens, rewards, finishes = evaluator.run(agent, **eval_cfg, work_dir=eval_dir, wandb_run=run)
                save_eval_statistics(eval_dir, lens, rewards, finishes)
                eval_dict = dict(mean_length=np.mean(lens),
                                 std_length=np.std(lens),
                                 mean_reward=np.mean(rewards),
                                 std_reward=np.std(rewards),
                                 success_mean=np.mean(finishes)
                                 )
            ...
            print_dict.update(eval_dict)
            logger.info(f"Evaluation: {eval_steps} {dict_to_str(print_dict)}")
            if world_rank == 0:
                tf_logger.log(eval_dict, n_iter=eval_steps, tag_name="test")

        if check_checkpoint.check(steps) and world_rank == 0:
            ...
            if run is not None:
                ...
                if isinstance(agent, VATPerception):
                    type = agent.mode  # trajectoryGenerator, trajectoryScorer, affordancePredictor

                artifact = wandb.Artifact(name=f"{type}_model_{standardized_ckpt_step}", type="model")
                ...
                if osp.isfile(osp.join(work_dir, 'meta.py')):
                    # upload meta.py file
                    artifact.add_file(osp.join(work_dir, 'meta.py'))
                else:
                    logger.error("META.PY FILE NOT FOUND!")

                run.log_artifact(artifact)
                logger.info(f'Uploaded checkpoint {standardized_ckpt_step} to WANDB')
    ...
    if n_checkpoint and world_rank == 0:
        ...
        if run is not None:
            ...
            artifact = wandb.Artifact(name=f"{type}_model_final", type="model")
            if os.path.isdir(model_path):
                artifact.add_dir(model_path)
            else:
                artifact.add_file(model_path)
            ...
            run.log_artifact(artifact)
            logger.info('Uploaded final checkpoint to WANDB')
