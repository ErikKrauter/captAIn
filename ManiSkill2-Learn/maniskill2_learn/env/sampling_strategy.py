...
class OneStepTransition(SamplingStrategy):
    ...
    def sample(self, batch_size, drop_last=True, auto_restart=True, traj_len=None):
...
class TStepTransition(SamplingStrategy):
    ...
    def __init__(self, horizon=1, **kwargs):
        ...
        """
        if horizon == 1:
            get_logger().warning("Please use OneStepTransition when horizon is 1!")
    ...
    def sample(self, batch_size, drop_last=True, auto_restart=True, traj_len=None):
        ...
        padded_size = max([len(_) for _ in ret]) if traj_len is None else traj_len
        ...
        ret = np.array(ret, dtype=np.int16)
        return ret, mask
