...
class ExtendedEnv(ExtendedWrapper):
    ...
    **ExtendedEnv should also be the outermost wrapper (except BufferAugmentedEnv)**
    ...
    def reset(self, *args, **kwargs):
        ...
        return GDict(obs).f64_to_f32(wrapper=False)  #xyz (1200,3), rgb (1200,3), frame_related_states (4,3), to_frames (2,4,4), state (67,)

    def step(self, action, *args, **kwargs):
        if isinstance(action, dict):
            action['action'] = self._process_action(action['action'])
...
class ManiSkill2_ObsWrapper(ExtendedWrapper, ObservationWrapper):
    def __init__(
        ...
        if isinstance(self.obs_mode, (list, tuple)):
            self.obs_mode = self.obs_mode[0]

        if self.obs_mode == "state":
            pass
        elif self.obs_mode == "rgbd":
            self.img_size = img_size
        elif self.obs_mode in ["pointcloud", "VAT", 'VAT-inference']:
    ...
    def reset(self, **kwargs):
        if self.fix_seed is not None:
            ...
            obs, reset_info = self.env.reset(**kwargs)  # agent{qpos, qvel, base_pose}, extra{mein zeug}, pointcloud{xyzw, rgb}
    ...
    def observation(self, observation):
        ...
        if self.obs_mode == "rgbd":
            ...
            if self.obs_frame in ["base", "world"]:
                ...
                # remove all points belonging to robot links
            if 'robot_seg' in observation['pointcloud'].keys():
                robot_mask = observation['pointcloud'].pop('robot_seg').flatten()
                robot_mask = robot_mask.astype(bool)
                no_robot_mask = ~robot_mask
                for k in observation["pointcloud"].keys():
                    observation["pointcloud"][k] = observation["pointcloud"][k][no_robot_mask]
            ...
            # unique_xyz = self.remove_duplicates(pointcloud["xyz"])
            # pointcloud["xyz"] = unique_xyz
            ...
            observation.pop('pointcloud')
            ret.pop('rgb')  # the base line does not use RGB values
            observation = flatten_state_dict(observation)  # this is needed else ManiSkill2-Learn complains
            ret['state'] = observation  # agent and extra concatenated

            return ret

        elif self.obs_mode == "pointcloud" or self.obs_mode == "VAT-inference":
            """
            ...
            # target_ids = [link.id for x.get_links() for x in self.env.get_articulations() if x.name=='panda_v2']
            ...
            if "target_joint_axis" in obs_extra_keys:  # for TurnFaucet
                joint_axis_info = (
                    to_origin.to_transformation_matrix()[:3, :3]
                    @ observation["extra"]["target_joint_axis"]
                )
                frame_related_states.append(joint_axis_info)
            if "target_link_pos" in obs_extra_keys:  # for TurnFaucet
                link_pos_info = apply_pose_to_point(
                    observation["extra"]["target_link_pos"], to_origin
                )
            ...
            #print(f'Observation is: {ret}')
            #print(f'Observation state shape is: {ret["state"].shape}')
    ...
    def remove_duplicates(self, pointcloud, tolerance=1e-4):
        from scipy.spatial import cKDTree
        """
            Remove duplicate points from a pointcloud.

            Parameters:
            - pointcloud: numpy.ndarray, the input pointcloud of shape (N, 3).
            - tolerance: float, the tolerance within which points are considered duplicates.

            Returns:
            - unique_pointcloud: numpy.ndarray, the deduplicated pointcloud.
            """
        # Build a KDTree for efficient nearest neighbor search
        pointcloud = pointcloud[pointcloud[..., 2] > 1e-4]  # ground filter
        tree = cKDTree(pointcloud)

        # Find pairs of points where the distance is less than the specified tolerance
        duplicates = tree.query_pairs(r=tolerance, output_type='ndarray')

        # Keep only one index from each pair of duplicates
        # This will mark all but one duplicate for removal
        indices_to_remove = set(duplicates[:, 1])

        # Create a mask to filter out duplicates
        mask = np.ones(len(pointcloud), dtype=bool)
        mask[list(indices_to_remove)] = False

        # Apply the mask to the pointcloud to remove duplicates
        unique_pointcloud = pointcloud[mask]

        return unique_pointcloud
...
class VATInfo_Wrapper(ExtendedWrapper):

    def step(self, action):

        obs, rew, terminated, truncated, info = self.env.step(action)
        pcd = obs.pop("xyz")
        # we want the point cloud information to be in the info
        # we do not want to use pointclouds as training inputs!
        info["pointcloud"] = pcd
        # this is needed else the internal logic of ManiSkill2-Learn complains that it received a dict
        obs = obs['state']
