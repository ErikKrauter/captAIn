...
from .wrappers import VATInfo_Wrapper, ManiSkill2_ObsWrapper, RenderInfoWrapper, ExtendedEnv, BufferAugmentedEnv, build_wrapper
...
def get_model_id_TurnFaucet(faucets):
    import os
    ...
    with open(textDocPath, 'r') as file:
        for line in file:
            line = line.strip()
            if line.isdigit():  # Check if the line contains only digits (positive integer)
                unique_model_ids.add(line)
            else:
                raise ValueError(f"Invalid entry found in file: {line}")

    # Convert set to list
    mylist = list(unique_model_ids)
    return mylist
...
def make_gym_env(
    ...
    **kwargs,
    ...
    if env_type == "mani_skill2":
        ...
        faucets = kwargs.pop('faucets', None)
        if 'render_mode' not in kwargs.keys():
            kwargs['render_mode'] = 'rgb_array'
        #video_dir = kwargs.pop('video_dir', None)
            
    # env = TimeLimit<OrderEnforcing<PointCloudObservationWraper<TurnFaucet>>>>
    # note the PointCloudObservationWrapper stems from ManiSkill2 and has no downsampling
    if 'TurnFaucet' in env_name:
        if kwargs.get("model_ids", None) is None and faucets is not None:
            # print(f"trainFaucets: {trainFaucets}")
            model_ids = get_model_id_TurnFaucet(faucets)
            kwargs["model_ids"] = model_ids
            # here I overwrite the max_episode_steps so it aligns with the number of waypoints we want our RL agent to train
            # on outputting
            if kwargs.get("num_waypoints", None):
                kwargs["max_episode_steps"] = kwargs["num_waypoints"]
    ...
    if isinstance(env, TimeLimit):
        env = env.env  # for some reason removeTimeLimit wrapper here again
        use_time_limit = True
    elif hasattr(env.unwrapped, "_max_episode_steps"):
    ...
    if env_type == "mani_skill2":
            ...
            #env = RecordEpisode(env, save_trajectory=False, save_video=True, output_dir=video_dir)
        env = RenderInfoWrapper(env)
        # coordinate transformation of pointcloud in obs_frame
        # everything is transformed in obs_frame (base pose, tcp pose, faucets joint axis...)
        # when obs_frame tcp pose from observation dict is used for transformation
        # sample goal points if n_goal_points>0
        # perform pointcloud downsampling
        # also returns transformation matrix between common frames (goal frame, ee frame, base frame)
        # the same wrapper is used for rgbd image processing
        ...
        if kwargs["obs_mode"] in ["VAT"]:
            env = VATInfo_Wrapper(env)
