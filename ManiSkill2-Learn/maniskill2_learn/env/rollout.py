...
class Rollout:
    def __init__(self, env_cfg, num_procs=20, with_info=False, seed=None, **kwargs):
        ...
        # this is a very complicated and intertwined function call to create the environment
        # in essence this return the TurnFaucet-v0 environment wrapped in many different wrappers
        # <UnifiedVectorEnvAPI<SingleEnv2VecEnv OR VectorEnv<ExtendedEnv<TimeLimit<ManiSkill2_ObsWrapper<RenderInfoWrapper<OrderEnforcing<PointCloudObservationWrapper<TurnFaucetEnv<TurnFaucet-vo>>>>>
        # important to note that tne PoinCloudObservationWrapper provided by ManiSkill2 will convert from depth images of the two cameras into a pointcloud and wordl coordinate frame
        # ManiSkill2_ObsWrapper will further convert the poincloud into specified observation frame, downsample the pointlcoud and do some additional stuff
        # TimeLimit is set to 200 steps for all environments
        # also important to note that a hardcoded seed is used somewhere in ManiSkill2 sapien base env
    ...
    def forward_with_policy(self, pi=None, num=1, on_policy=False, replay=None):
        ...
        if on_policy:
            ...
            ret = []
            # print(f"collecting trajectory {self.forward_counter}")
            start = time.time()
            for i in range(num // self.num_envs):  # num is the number of steps to do in environment
                action = get_actions()
