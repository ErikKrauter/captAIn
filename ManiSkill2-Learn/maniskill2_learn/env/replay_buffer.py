...
class ReplayMemory:
    ...
    def sample(self, batch_size, auto_restart=True, drop_last=True, traj_len=None):
        if self.dynamic_loading and not drop_last:
            assert self.capacity % batch_size == 0

        batch_idx, is_valid = self.sampling.sample(batch_size, drop_last=drop_last, auto_restart=auto_restart and not self.dynamic_loading, traj_len=traj_len)
        if batch_idx is None:
            # without replacement only
            if auto_restart or self.dynamic_loading:
                items = self.file_loader.get()
                #get_logger().error(f'items: {items}')
                if items is None:
                    #get_logger().error('items is NONE')
                    return None
                ...
                return None
        #get_logger().error(f'batch idx {batch_idx}')
        ret = self.memory.take(batch_idx)
        ret["is_valid"] = is_valid
        return ret

    def mini_batch_sampler(self, batch_size, drop_last=False, auto_restart=False, max_num_batches=-1, traj_len=None):
        if self.sampling is not None:
        ...
        for i in count(1):
            if i > max_num_batches and max_num_batches != -1:
                break
            items = self.sample(batch_size, auto_restart, drop_last, traj_len=traj_len)
            if items is None:
