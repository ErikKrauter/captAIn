...
class UnifiedVectorEnvAPI(ExtendedWrapper):
    ...
    def step(self, actions, idx=None):
        ...
        if isinstance(actions, np.ndarray):
            ...
            assert len(actions['action']) == len(idx)
...
class SingleEnv2VecEnv(VectorEnvBase):
    ...
    def step(self, actions, idx=None):
        if isinstance(actions, dict):
            ...
            #print(f'isinstance(actions, dict)')
            return self._unsqueeze(self._env.step(dict(action=actions['action'][0], normal=actions.get('normal', None),
                                                       open_loop_trajectory=actions.get('open_loop_trajectory', None),
                                                       contact_point_feature=actions.get('contact_point_feature', None))))
        elif isinstance(actions, np.ndarray):
            #print(f'isinstance(actions, np.ndarray)')
...
class VectorEnv(VectorEnvBase):
    ...
    def __init__(self, env_cfgs, seed=None, work_dir=None, **kwargs):
    ...
    def step(self, actions, idx=None):
        ...
        if isinstance(actions, dict):
            ...
            # environment.
            actions = [{k: v for k, v in zip(actions.keys(), action_values)} for action_values in
                       zip(*actions.values())]
...
class SapienThreadEnv(VectorEnvBase):
    ...
    def step_random_actions(self, num):
        ...
        while num > 0:
            ...
            num -= num_i
            if np.any(term_or_truncs):
                self.reset(idx=np.where(term_or_truncs[..., 0])[0])
