...
def merge_h5_trajectory(h5_files, output_name, num=-1):
    with h5py.File(output_name, "w") as f:
        index, neg_index, neg_cp_index, meta = 0, 0, 0, None
            ...
            for i in range(len(h5_keys)):
                if f'traj_{i}' in h5_keys:
                    ...
                    if num != -1 and index >= num:
                        break
                if f'traj_{i}_neg' in h5_keys:
                    h5.copy(f"traj_{i}_neg", f, f"traj_{neg_index}_neg")
                    neg_index += 1
                    if num != -1 and neg_index >= num:
                        break
                if f'traj_{i}_neg_cp' in h5_keys:
                    h5.copy(f"traj_{i}_neg_cp", f, f"traj_{neg_cp_index}_neg_cp")
                    neg_cp_index += 1
                    if num != -1 and neg_index >= num:
                        break
            if num != -1 and index >= num:
                break
        get_logger().info(f"Total number of trajectories {index+neg_index+neg_cp_index}")
        get_logger().info(f"Total number of negative trajectories {neg_index+neg_cp_index}")
        get_logger().info(f"Number of trajectories with non-contact point {neg_cp_index}")
        get_logger().info(f"Number of other negative trajectories {neg_index}")
