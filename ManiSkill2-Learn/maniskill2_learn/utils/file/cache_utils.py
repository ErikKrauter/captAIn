...
def is_h5_traj(h5):
    ...
    keys = list(h5.keys())
    pattern = r"traj_\d+$"
    contains_traj_num_regex = any(re.match(pattern, string) for string in keys)
    return contains_traj_num_regex
    ...
    if file_suffix == "h5":
        if is_h5_traj(filename):
            ...
            with File(filename, "r") as f:
                l = list(f.keys())
                key = next((string for string in l if re.match(pattern, string)), None)
