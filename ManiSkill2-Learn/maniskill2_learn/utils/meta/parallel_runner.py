...
class Worker(Process):
    ...
    def __init__(self, cls, worker_id, worker_seed=None, daemon=True, mem_infos=None, is_class=True, *args, **kwargs):
        ...
        self.cls = cls
        # print(f"worker pid {os.getpid()} has cls {self.cls}")
    ...
    def run(self):
        ...
        if self.shared_memory.value:
            ...
            # print(f"worker pid {os.getpid()} has self.shared_memory.value: {self.shared_memory.value}")
            # print(f"worker pid {os.getpid()} has func: {func}")
        ...
        if self.worker_seed is not None:
            ...
            if self.is_class and hasattr(func, "seed"):
                # For gym environment
                print(self.worker_id, " setting environment seed to ", self.worker_seed)
    ...
    def _send_info(self, info):
        ...
        """
        if self.item_in_pipe.value is not 0:
            print("self.item_in_pipe.value is not 0. Keep running.")
        # self.item_in_pipe.value = 0
