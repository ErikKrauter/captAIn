...
agent_cfg = dict(
    type="TD3",
    batch_size=64,  # Using multiple gpus leads to larger effective batch size, which can be crucial for SAC training
    gamma=0.99,
    update_coeff=0.005,
    ...
    actor_cfg=dict(
        ...
        nn_cfg=dict(
            ...
            visual_nn_cfg=dict(type="PointNet2", hparams={'feat_dim': 128}),  # feat_dim is the ouput dim of the pointnet, it must align with mlp_spec below
            mlp_cfg=dict(
                type="LinearMLP",
                norm_cfg=None,
                mlp_spec=["128 + agent_shape", 256, 256, "action_shape * 2"],
    ...
    critic_cfg=dict(
        ...
        nn_cfg=dict(
            ...
            mlp_cfg=dict(
                type="LinearMLP", norm_cfg=None, mlp_spec=["128 + agent_shape + action_shape", 256, 256, 1], inactivated_output=True, zero_init_output=True
            ),
        ),
