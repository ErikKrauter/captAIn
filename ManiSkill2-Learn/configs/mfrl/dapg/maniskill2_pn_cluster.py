...
agent_cfg = dict(
    ...
    demo_replay_cfg=dict(
        type="ReplayMemory",
        capacity=int(0.4e4), #int(2e4)
        num_samples=-1,
        cache_size=int(0.4e4), #int(2e4)
        dynamic_loading=True,
        synchronized=False,
        keys=["obs", "actions", "dones", "episode_dones"],
        buffer_filenames=[
            "ManiSkill2/demos/v0/rigid_body/TurnFaucet-v0/processedTrainingDemos/training_trajectories_merged.none.pd_ee_delta_pose_pointcloud_shuffled.h5",
            # this path is relative to the directory from which the training script is started which is the root of the
            # project namely MasterThesis
        ],
    ),
...
train_cfg = dict(
    on_policy=True,
    total_steps=int(5e6), #int(5e6)
    warm_steps=0,
    n_steps=int(0.4e4), #int(2e4)
    n_updates=1,
    n_eval=int(5e6), #int(5e6) # will perform eval every n_eval steps
    n_log=1000,  # will log data every n_log steps to tensorboard
    n_checkpoint=int(1e6), #int(1e6) # will save model every n_checkpoint steps
    ep_stats_cfg=dict(
        info_keys_mode=dict(
...
eval_cfg = dict(
    ...
    trainFaucets="faucetTrainingDataSet.txt",
    holdOutFaucets="faucetTHoldOutDataSet.txt"
)
