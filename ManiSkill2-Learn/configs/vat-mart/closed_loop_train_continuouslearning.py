...
train_cfg = dict(
    ...
    n_steps=160,  # collect N transitions in environment. MUST BE DEVISIBLE BY num_procs, because n_steps is spread over vectorized environment
    n_updates=2,  # how many updates are done after having collected n_steps interations. Each update uses batch_size many samples
    n_log=800,  # log to tensorboard every n_log transitions. Can only log when outside of interaction/training loop
    print_steps=2,  # in episodes: break out of interaction/training loop every N episodes for printing to the terminal
    ep_stats_cfg=dict(
        info_keys_mode=dict(
