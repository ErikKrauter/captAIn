agent_cfg = dict(
    ...
    trajectory_generator_checkpoint_path='Experiments/Gen_UseNoReverseContactsImproved/models/model_17500.ckpt',
    trajectory_scorer_checkpoint_path='Experiments/Scor_UseNoReverseContactsImproved/models/model_17500.ckpt',
    batch_size=64,
    ...
    affordance_predictor_cfg=dict(
        ...
        topk=5,
        backbone_cfg=dict(type="PointNet2", hparams={'feat_dim': 128}),
        mlp_cp_cfg=dict(
    ...
    trajectory_generator_cfg=dict(
        ...
        mlp_traj_cfg=dict(  # three layer MLP, Called TrajEncoder in Baseline
            ...
            # (num_steps+1) * waypoint dim, 128, 128, trajectory features dim
            inactivated_output=True,
            linear_init_cfg=dict(
                type="xavier_init",
                gain=1,
...
replay_cfg = dict(
    type="ReplayMemory",
    sampling_cfg=dict(type="TStepTransition", horizon=-1, with_replacement=False),  # horizon=-1 means we sample whole trajectories
    capacity=-1,
    #cache_size=5000,
    num_samples=-1,
    ...
    buffer_filenames=[
        "DatasetsAndModels/RLTrajectories/SAC-VAT-TaretPose-Improved/TrainAugmentation/test/trajectory.h5",
    ],
)
...
eval_cfg = dict(
    ...
    eval_replay=dict(
        ...
        keys=["obs", "actions", "dones", "episode_dones", "infos"],
        buffer_filenames=[
            "DatasetsAndModels/RLTrajectories/SAC-VAT-TaretPose-Improved/ValAugmentation/test/trajectory.h5"
        ],
    ),
)
